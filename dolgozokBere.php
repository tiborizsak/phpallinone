...
//            echo $adatok[4]["orszag"] . ": " . $adatok[4]["szazalek"] . "%<br>"; // pl. Így hivatkozunk Venezuela adataira
        ...
        */
        ...
        $dolgozokNeveStr = "Kovács Béla,Horváth Lajos,Takácsi Előd,Solymár Tódor,Latinka Alajos,Obester János,Fankovics Béláné,Kultur János,Ópusztai Jenő,Kovács Franciska,Félig László,Herman Géza,Bóczay Elemérné,Teljes Ödön,Deli Vitéz";
        $dolgozokTelepHStr = "Bp.,Miskolc,Szeged,Veszprém,Bp.,Szolnok,Miskolc,Bp.,Veszprém,Szeged,Bp.,Szolnok,Miskolc,Szeged,Veszprém";
        $dolgozokBeosztStr = "Targoncás,Targoncás,Targoncás,Targoncás,Targoncás,Targoncás,Targoncás,Darukezelő,Darukezelő,Darukezelő,Darukezelő,Darukezelő,Darukezelő,Darukezelő,Darukezelő";
        $dolgozokBereStr = "215240,218570,214680,219880,225600,213250,222150,227050,225990,215110,226520,221570,216890,224660,219350";
        
        $adatok = array("dolgozokN" => explode(",", $dolgozokNeveStr), 
                        "dolgozokTH" => explode(",", $dolgozokTelepHStr),
                        "dolgozokBO" => explode(",", $dolgozokBeosztStr), 
                        "dolgozokF" => explode(",", $dolgozokBereStr)
            );
        ...
        foreach ($adatok["dolgozokTH"] as $i => $varosok){
                
                     if($varosok === "Bp."){
                         $BPdolgozok++;
                         $BPosszkereset += $adatok["dolgozokF"][$i];
                         
                     }
                     
                     if($varosok === "Miskolc"){
                         $Miskolcossz += $adatok["dolgozokF"][$i];
                     }
                     
                     
            }
            
        foreach ($adatok["dolgozokBO"] as $i => $beosztas){
                         if($varosok === "Szeged" && $beosztas = "Darukezelő"){
                             $Szegedaru += $adatok["dolgozoF"][$i]
                             if ();
                         }
                     }    
        ...
        $BPatlagber = $BPosszkereset / $BPdolgozok;
        
        echo "A budapesti dolgozók átlagbére: ".number_format($BPatlagber,0)." Ft.<br>";
        echo "A Miskolciak össz fizetése: ".number_format($Miskolcossz,0)." Ft.<br>";
        echo print_r($Szegedaru);
        //var_dump($adatok);
        ?>
    </body>
</html>
